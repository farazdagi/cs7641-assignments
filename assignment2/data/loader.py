...
class DataLoader(ABC):
    ...
    def scale_standard(self):
        self.log("Applying StandardScaler on features/training_x/testing_x")
    ...
    def build_train_test_split(self, test_size=0.3):
        if not self.training_x and not self.training_y and not self.testing_x and not self.testing_y:
            ...
            self.log("Splitting train and test sets, test size: {}".format(test_size))
            self.log("training_x: {}, training_y: {}, testing_x: {}, testing_y: {}".format(
                self.training_x.shape,
                self.training_y.shape,
                self.testing_x.shape,
                self.testing_x.shape,
            ))
...
class SpamData(DataLoader):
    ...
    def _preprocess_data(self):
        self.log("No special pre-processing required")

    def pre_training_adjustment(self, train_features, train_classes):
        self.log("No special pre-training adjustments required")
        return train_features, train_classes
